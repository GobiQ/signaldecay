...
class OptionSpec:
    ...
    iv: float           # implied vol (annualized, e.g. 0.25)
    r: float = 0.00     # risk-free (annual)
    q: float = 0.00     # div yield (annual)

def _bs_price(kind: str, S: float, K: float, tau: float, iv: float, r: float = 0.0, q: float = 0.0):
    """Black-Scholes European price."""
    if tau <= 0 or iv <= 0 or S <= 0 or K <= 0:
        # At expiry, intrinsic only (or degenerate)
        if tau <= 0:
            if kind == "call":
                return max(S - K, 0.0)
            else:
        ...
        return 0.0
    sig = iv
    d1 = (log(S/K) + (r - q + 0.5*sig*sig)*tau) / (sig*sqrt(tau))
    d2 = d1 - sig*sqrt(tau)
    if kind == "call":
...
def _nearest_strike_by_moneyness(S: float, moneyness_pct: float):
    """Return K given S and % moneyness (e.g., +5 => 5% OTM call; -5 => 5% ITM call for puts)."""
    return S * (1.0 + moneyness_pct/100.0)

def _strike_by_target_delta(kind: str, S: float, tau: float, iv: float, r: float=0.0, q: float=0.0, target_delta: float=0.30):
    """Solve strike for given Black-Scholes delta by simple bracket/binary search."""
    # Bracket around S +/- 50% as a crude bound
    lo, hi = S*0.5, S*1.5
    for _ in range(40):
        K = 0.5*(lo+hi)
        # Call delta = e^{-q tau} N(d1); Put delta = -e^{-q tau} N(-d1)
        if tau <= 0 or iv <= 0: 
            return K
        sig = iv
        d1 = (log(S/K) + (r - q + 0.5*sig*sig)*tau) / (sig*sqrt(tau))
...
def price_long_single(S_entry: float, S_exit: float, days_held: int, spec: OptionSpec,
                      iv_entry: float, iv_exit: float, comm_per_contract: float=0.0, half_spread_bps: float=0.0):
    """Return pct P&L of a long single option over the holding window."""
    tau_entry = max(spec.DTE, 1)/252.0
    tau_exit  = max(spec.DTE - days_held, 0)/252.0
    prem_in   = _bs_price(spec.kind, S_entry, spec.K, tau_entry, iv_entry, spec.r, spec.q)
    prem_out  = _bs_price(spec.kind, S_exit,  spec.K, tau_exit,  iv_exit,  spec.r, spec.q)
    # Apply costs: one round-trip half-spread each side + commissions
    spread_cost_in  = prem_in  * (half_spread_bps/1e4)
    spread_cost_out = prem_out * (half_spread_bps/1e4)
    cash_in  = prem_in  + spread_cost_in  + comm_per_contract
    cash_out = prem_out - spread_cost_out - comm_per_contract
    pnl = cash_out - cash_in
    return pnl / max(cash_in, 1e-9)

def price_vertical_call_debit(S_entry, S_exit, days_held, long_spec: OptionSpec, short_K_offset: float,
                              iv_entry, iv_exit, comm_per_contract=0.0, half_spread_bps=0.0):
    """Long call at K; short call at K*(1+offset%) (offset can be 0.05 for +5%)."""
    K_long  = long_spec.K
    K_short = K_long * (1.0 + short_K_offset)
    # Long leg
    pnl_long = price_long_single(S_entry, S_exit, days_held, long_spec, iv_entry, iv_exit, comm_per_contract, half_spread_bps)
    # Short leg (sell at entry, buy back at exit) => negative of long with K_short
    short_spec = OptionSpec(kind="call", K=K_short, DTE=long_spec.DTE, iv=long_spec.iv, r=long_spec.r, q=long_spec.q)
    # Flip entry/exit because it's short: profit when premium falls
    tau_entry = max(short_spec.DTE, 1)/252.0
    tau_exit  = max(short_spec.DTE - days_held, 0)/252.0
    prem_in_short  = _bs_price("call", S_entry, K_short, tau_entry, iv_entry, short_spec.r, short_spec.q)  # we SELL this
    prem_out_short = _bs_price("call", S_exit,  K_short, tau_exit,  iv_exit,  short_spec.r, short_spec.q)  # we BUY back
    spread_in  = prem_in_short  * (half_spread_bps/1e4)
    spread_out = prem_out_short * (half_spread_bps/1e4)
    cash_in  = prem_in_short  - spread_in  - comm_per_contract  # credit received
    cash_out = prem_out_short + spread_out + comm_per_contract  # debit to close
    pnl_short = cash_in - cash_out
    # Combine legs; normalize by net debit at entry for % return
    # Compute initial net debit cost for the spread:
    prem_long_in = _bs_price("call", S_entry, K_long,  tau_entry, iv_entry, long_spec.r, long_spec.q)
    net_debit_in = (prem_long_in + prem_long_in*(half_spread_bps/1e4) + comm_per_contract) - (prem_in_short - spread_in - comm_per_contract)
    total_pnl = pnl_long*max(prem_long_in,1e-9) + pnl_short  # pnl_long is %; convert to $ by *premium
    return total_pnl / max(net_debit_in, 1e-9)

def price_vertical_put_debit(S_entry, S_exit, days_held, long_spec: OptionSpec, short_K_offset: float,
                             iv_entry, iv_exit, comm_per_contract=0.0, half_spread_bps=0.0):
    """Long put at K; short put at K*(1-offset%) (offset can be 0.05 for -5%)."""
    K_long  = long_spec.K
    K_short = K_long * (1.0 - short_K_offset)  # Short put is lower strike
    # Long leg
    pnl_long = price_long_single(S_entry, S_exit, days_held, long_spec, iv_entry, iv_exit, comm_per_contract, half_spread_bps)
    # Short leg (sell at entry, buy back at exit)
    short_spec = OptionSpec(kind="put", K=K_short, DTE=long_spec.DTE, iv=long_spec.iv, r=long_spec.r, q=long_spec.q)
    tau_entry = max(short_spec.DTE, 1)/252.0
    tau_exit  = max(short_spec.DTE - days_held, 0)/252.0
    prem_in_short  = _bs_price("put", S_entry, K_short, tau_entry, iv_entry, short_spec.r, short_spec.q)  # we SELL this
    prem_out_short = _bs_price("put", S_exit,  K_short, tau_exit,  iv_exit,  short_spec.r, short_spec.q)  # we BUY back
    spread_in  = prem_in_short  * (half_spread_bps/1e4)
    spread_out = prem_out_short * (half_spread_bps/1e4)
    cash_in  = prem_in_short  - spread_in  - comm_per_contract  # credit received
    cash_out = prem_out_short + spread_out + comm_per_contract  # debit to close
    pnl_short = cash_in - cash_out
    # Combine legs; normalize by net debit at entry for % return
    prem_long_in = _bs_price("put", S_entry, K_long,  tau_entry, iv_entry, long_spec.r, long_spec.q)
    net_debit_in = (prem_long_in + prem_long_in*(half_spread_bps/1e4) + comm_per_contract) - (prem_in_short - spread_in - comm_per_contract)
    total_pnl = pnl_long*max(prem_long_in,1e-9) + pnl_short
    return total_pnl / max(net_debit_in, 1e-9)

def price_straddle(S_entry, S_exit, days_held, spec: OptionSpec, iv_entry, iv_exit, 
                   comm_per_contract=0.0, half_spread_bps=0.0):
    """Long straddle: long call + long put at same strike."""
    # Call leg
    call_spec = OptionSpec(kind="call", K=spec.K, DTE=spec.DTE, iv=spec.iv, r=spec.r, q=spec.q)
    pnl_call = price_long_single(S_entry, S_exit, days_held, call_spec, iv_entry, iv_exit, comm_per_contract, half_spread_bps)
    # Put leg  
    put_spec = OptionSpec(kind="put", K=spec.K, DTE=spec.DTE, iv=spec.iv, r=spec.r, q=spec.q)
    pnl_put = price_long_single(S_entry, S_exit, days_held, put_spec, iv_entry, iv_exit, comm_per_contract, half_spread_bps)
    # Total cost is sum of both premiums
    tau_entry = max(spec.DTE, 1)/252.0
    prem_call = _bs_price("call", S_entry, spec.K, tau_entry, iv_entry, spec.r, spec.q)
    prem_put = _bs_price("put", S_entry, spec.K, tau_entry, iv_entry, spec.r, spec.q)
    total_cost = prem_call + prem_put + 2*comm_per_contract + (prem_call + prem_put)*(half_spread_bps/1e4)
    # Convert percentage P&L to dollar P&L and combine
    pnl_dollar = pnl_call * prem_call + pnl_put * prem_put
    return pnl_dollar / max(total_cost, 1e-9)
...
with st.sidebar:
    ...
    # Options Mode Controls
    st.subheader("Options Mode")
    
    payout_source = st.radio(
        "Payout source",
        ["Underlying", "Options strategy"],
        index=0,
        help="Underlying: Use traditional underlying asset returns. Options strategy: Use options P&L for signal events."
    )
    
    # Options setup (shown when Options strategy is selected)
    if payout_source == "Options strategy":
        with st.expander("Options Setup", expanded=True):
            strategy = st.selectbox(
                "Strategy",
                ["Long Call", "Long Put", "Vertical Call (debit)", "Vertical Put (debit)", "Covered Call", "Cash-Secured Put", "Straddle", "Strangle", "Calendar (call/put)"],
                index=2,  # Default to Vertical Call (debit)
                help="Options strategy to execute on signal events"
            )
            
            dte_days = st.number_input(
                "DTE at entry (days)",
                min_value=1, max_value=365, value=30, step=1,
                help="Days to expiry when entering the options position"
            )
            
            exit_rule = st.radio(
                "Exit rule",
                ["Fixed horizon H", "Hold to expiry", "Earlier of H or expiry"],
                index=2,
                help="When to exit the options position"
            )
            
            strike_rule = st.radio(
                "Strike selection",
                ["By delta", "By moneyness (%)", "Nearest-to-money"],
                index=0,
                help="Method for selecting option strikes"
            )
            
            if strike_rule == "By delta":
                delta_target = st.number_input(
                    "Target delta",
                    min_value=0.05, max_value=0.95, value=0.30, step=0.05,
                    help="Target delta for strike selection (0.30 = 30 delta)"
                )
                moneyness_pct = None
            elif strike_rule == "By moneyness (%)":
                moneyness_pct = st.number_input(
                    "Moneyness (%)",
                    min_value=-20.0, max_value=20.0, value=0.0, step=0.5,
                    help="Moneyness percentage (+5 = 5% OTM for calls, -5 = 5% ITM for puts)"
                )
                delta_target = None
            else:
                moneyness_pct = None
            ...
            iv_model = st.radio(
                "IV model",
                ["Flat IV (%)", "Proxy (VIX/VXN)"],
                index=0,
                help="How to model implied volatility"
            )
            
            if iv_model == "Flat IV (%)":
                flat_iv = st.number_input(
                    "Flat IV (%)",
                    min_value=5.0, max_value=200.0, value=25.0, step=1.0,
                    help="Flat implied volatility percentage"
                ) / 100.0
            else:
                flat_iv = 0.25  # Default fallback
            
            # Costs
            st.markdown("**Costs**")
            comm_per_contract = st.number_input(
                "Commission per contract ($)",
                min_value=0.0, max_value=10.0, value=1.0, step=0.1,
                help="Commission cost per options contract"
            )
            
            half_spread_bps = st.number_input(
                "Bid-ask half-spread (bps)",
                min_value=0, max_value=1000, value=20, step=5,
                help="Half-spread in basis points (20 bps = 0.2%)"
            )
            
            slippage_pct = st.number_input(
                "Slippage (%)",
                min_value=0.0, max_value=5.0, value=0.0, step=0.1,
                help="Additional slippage percentage"
            )
            
            # Risk controls
            st.markdown("**Risk Controls**")
            max_loss_cap = st.checkbox(
                "Max loss cap (stop at -premium for longs)",
                value=False,
                help="Stop loss at 100% premium loss for long positions"
            )
            
            profit_take_pct = st.number_input(
                "Profit take (%)",
                min_value=0.0, max_value=1000.0, value=0.0, step=10.0,
                help="Take profit at this percentage gain (0 = no profit taking)"
            )
            
            early_assignment = st.checkbox(
                "Early assignment (on/off)",
                value=False,
                help="Enable early assignment modeling (default off for European simplification)"
            )
            
            # For spreads, add second leg parameters
            if "Vertical" in strategy:
                short_K_offset = st.number_input(
                    "Short leg offset (%)",
                    min_value=0.0, max_value=50.0, value=5.0, step=0.5,
                    help="Short leg strike offset as percentage (5% = short call 5% higher than long call)"
                )
            else:
                short_K_offset = 0.0
        ...
        # Default values when not in options mode
        strategy = "Long Call"
        dte_days = 30
        exit_rule = "Earlier of H or expiry"
        strike_rule = "By delta"
        delta_target = 0.30
        moneyness_pct = None
        flat_iv = 0.25
        comm_per_contract = 1.0
        half_spread_bps = 20
        slippage_pct = 0.0
        max_loss_cap = False
        profit_take_pct = 0.0
...
if edge_mode == "Fixed horizon (days)":
    ...
    if payout_source == "Options strategy":
        # For options mode, calculate options P&L for each signal event
        opt_rets = []
        idx = prices.index
        for i, is_sig in enumerate(prices['signal'].values):
            if not is_sig or i+1 >= len(idx):
                opt_rets.append(np.nan)
                continue
            entry_i = i+1
            exit_i = min(i+1+horizon, len(idx)-1)
            S_in = float(prices['close_tgt'].iloc[entry_i])
            S_out = float(prices['close_tgt'].iloc[exit_i])

            # Build strike
            tau_entry_days = int(dte_days)
            tau_exit_days = max(tau_entry_days - (exit_i - entry_i), 0)
            tau_entry = tau_entry_days/252.0
            iv_in = flat_iv
            iv_out = flat_iv  # Start simple - keep IV constant

            if strike_rule == "By delta":
                if strategy in ["Long Call", "Vertical Call (debit)"]:
                    K = _strike_by_target_delta("call", S_in, tau_entry, iv_in, target_delta=delta_target)
                elif strategy in ["Long Put", "Vertical Put (debit)"]:
                    K = _strike_by_target_delta("put", S_in, tau_entry, iv_in, target_delta=delta_target)
                else:  # Straddle, etc. - use ATM
                    K = S_in
            elif strike_rule == "By moneyness (%)":
                K = _nearest_strike_by_moneyness(S_in, moneyness_pct)
            else:  # nearest-to-money
                K = S_in

            # Set option kind based on strategy
            if strategy in ["Long Call", "Vertical Call (debit)"]:
                option_kind = "call"
            elif strategy in ["Long Put", "Vertical Put (debit)"]:
                option_kind = "put"
            else:  # Straddle, etc. - use call as default
                option_kind = "call"

            spec = OptionSpec(kind=option_kind, K=K, DTE=tau_entry_days, iv=iv_in, r=rf_rate, q=div_yield)

            if strategy == "Long Call":
                pnl_pct = price_long_single(S_in, S_out, exit_i-entry_i, spec, iv_in, iv_out,
                                          comm_per_contract, half_spread_bps)
            elif strategy == "Long Put":
                spec.kind = "put"
                pnl_pct = price_long_single(S_in, S_out, exit_i-entry_i, spec, iv_in, iv_out,
                                          comm_per_contract, half_spread_bps)
            elif strategy == "Vertical Call (debit)":
                pnl_pct = price_vertical_call_debit(S_in, S_out, exit_i-entry_i, spec, short_K_offset/100.0,
                                                  iv_in, iv_out, comm_per_contract, half_spread_bps)
            elif strategy == "Vertical Put (debit)":
                spec.kind = "put"
                pnl_pct = price_vertical_put_debit(S_in, S_out, exit_i-entry_i, spec, short_K_offset/100.0,
                                                 iv_in, iv_out, comm_per_contract, half_spread_bps)
            elif strategy == "Straddle":
                pnl_pct = price_straddle(S_in, S_out, exit_i-entry_i, spec, iv_in, iv_out,
                                       comm_per_contract, half_spread_bps)
            else:
                pnl_pct = np.nan  # Fill out other strategies similarly

            opt_rets.append(pnl_pct)

        prices['event_ret'] = np.array(opt_rets, dtype=float)
    else:
        # Traditional underlying returns
    ...
    if payout_source == "Options strategy":
        edge_mode_note = f"Options strategy edge over {eval_window} trading days; forward horizon = {horizon} days; strategy = {strategy}."
    else:
...
with col1:
    ...
    if build_equity:
        if payout_source == "Options strategy":
            st.subheader("Options Strategy Equity Curve")
        else:
        ...
        if payout_source == "Options strategy":
            # For options mode, use options P&L for strategy returns
            # Create strategy returns from options P&L on signal days, 0% on non-signal days
            strat_ret = pd.Series(0.0, index=prices.index)
            
            # Fill in options P&L on signal days
            for i, (date, is_signal) in enumerate(prices['signal'].items()):
                if is_signal and i < len(prices['event_ret']):
                    if not pd.isna(prices['event_ret'].iloc[i]):
                        strat_ret.iloc[i] = prices['event_ret'].iloc[i]
            
            # Comparison returns (cash/bonds)
            ...
            eq_df = pd.DataFrame({
                f'Options Strategy ({strategy})': eq_strat,
                f'Options Strategy (Tax {tax_rate:.0f}%)': eq_strat_tax,
...
with col2:
    ...
    if use_precond_only:
        ...
        if payout_source == "Options strategy":
            st.write(f"Signal on **{source_ticker}**, options strategy: **{strategy}** on **{effective_target_ticker}**.")
            st.write(f"**Options setup:** {dte_days} DTE, {strike_rule}, IV={flat_iv:.1%}, costs=${comm_per_contract}/contract + {half_spread_bps}bps spread")
        else:
...
st.markdown(
    ...
    **Methodology**
    ...
    - **Options mode**: Uses Black-Scholes pricing with configurable IV, strike selection (delta/moneyness/ATM), and realistic costs (commissions, spreads). Strategies include long calls/puts, vertical spreads, and more.
    """
)
